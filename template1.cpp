...
int main() {
	...
	//fill-in code DONE
	int* theMostActUser = (*A).activeUsers();
	cout << "Users ranked by Activity: " << endl;
	for (int i = 0; i < (*A).getNumRows(); i++)
		cout << theMostActUser << " ";
	cout << endl;

	// Call the destructors DONE
	delete A;
	delete[] influentialityVector;
	delete[] theMostActUser;
